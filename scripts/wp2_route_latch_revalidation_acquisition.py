#!/usr/bin/env python3
"""Run the fixed four-slot §45 observational telemetry acquisition."""
...
def write_manifest(path: Path, slots: list[dict]) -> None:
    collector.atomic_json(
        ...
        {
            "schema_version": 1,
            "design": "s45_route_latch_revalidation_acquisition",
...
def main() -> int:
    ...
    parser.add_argument("--work-dir", type=Path, default=Path(".tmp/s45_route_latch_revalidation"))
    args = parser.parse_args()
    root = Path(__file__).resolve().parents[1]
    if collector.game_running():
        raise SystemExit("ABORT: Brotato is already running; attribute and stop it before §45")
    ...
    if manifest_path.is_file():
        ...
        if existing.get("fixed_slots") != list(SLOTS):
            raise RuntimeError("existing manifest does not match fixed §45 slots")
    ...
    for slot in SLOTS:
        ...
        if run_id is None:
            if state_path.exists():
                ...
                raise RuntimeError(
                    ...
                    f"{state.get('stop_reason')!r}); automatic top-up is forbidden"
                )
            if len(slots) >= slot:
            ...
            print(f"SLOT {slot}/4 arm=I: launching exactly one bounded run", flush=True)
        ...
        print(f"SLOT {slot}/4 arm=I: certified {run_id}", flush=True)

    arm_and_read_back(False)
    print(f"§45 ACQUISITION COMPLETE: 4/4; manifest={manifest_path}")
    return 0
