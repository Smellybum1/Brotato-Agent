"""Build the ``combat_dagger_r3`` corrective dataset (WP2 M4 paired-eval close-out).

Thin round-3 parameterization of ``scripts/wp2_build_combat_dagger_r2.py`` / r1:
the identical FROZEN pipeline and gates, reusing the pure helpers unchanged
...
event-balanced weights; ``git_head_commit`` / ``sha256_file``; the Stage-A
audit primitives ``audit_run`` / ``reconcile_run`` / ``load_sidecar``). Round-1
and round-2 outputs are frozen and never touched.

Source arm = the six ``bc_v3_a_s1`` live runs that constitute the v3a side of the
M4 paired evaluation (design .tmp/wp2_m4_dagger_design.md sect. 9). Per the
predeclared rule these runs enter the round-3 corrective pool as VALID DAgger
data under bc_v3_a regardless of the (retain bc_v2_f) promotion verdict.

Round-3 specifics vs round 2:
  * NO dedicated collection-campaign state or Stage-A audit file exists (these
    were eval runs, not a collection campaign), so this builder runs the Stage-A
    audit INLINE via the reused r1/r2 primitives rather than reading an audit
    JSON. Identical gates (integrity + >=0.95 student control + triple
    reconciliation).
  * TWO v3a sidecar logs, one act stream each (single handshake / connection):
      - ``student_sidecar_dagger_r3.jsonl``   -> the bc_v3_a smoke run,
      - ``student_sidecar_paired_eval.jsonl`` -> the 5 fresh paired-eval runs
        (one contiguous seq space across all five, no per-run reset).
    Each run's triple reconciliation joins against ITS log's act map.
  * manifest ``student_policy = bc_v3_a_s1`` (model sha 2979C36A...), dataset id
    ``combat_dagger_r3`` -> ``datasets/combat_dagger_r3/``.
...
Exit codes: 0 = built, all gates pass; 1 = a build gate / audit gate failed;
...
# Reuse the frozen r1 build helpers (they in turn reuse the frozen combat_obs_v1
# pipeline) and the frozen r1/r2 Stage-A audit primitives. Nothing round-1/2
# identity-bearing (model shas, dataset ids) is imported.
...
BUILDER_REL = "scripts/wp2_build_combat_dagger_r3.py"
RUNS_ROOT = Path("C:/Users/moxhe/AppData/Roaming/Brotato/brotato_agent/runs")
REGISTRY_PATH = ROOT / "models" / "registry" / "bc_v3_a_s1.json"
SIDECAR_SMOKE = ROOT / ".tmp" / "student_sidecar_dagger_r3.jsonl"
SIDECAR_PAIRED = ROOT / ".tmp" / "student_sidecar_paired_eval.jsonl"
DATASET_DIR = ROOT / "datasets" / "combat_dagger_r3"
REPORT_MD = ROOT / "reports" / "wp2" / "dagger_r3_dataset_report.md"
REPORT_JSON = ROOT / "reports" / "wp2" / "dagger_r3_dataset_report.json"

STUDENT_POLICY = "bc_v3_a_s1"
EXPECTED_MODEL_SHA256 = "2979C36A4A6D833C85BE128A1599C379E236B69EDA45F0563369780BEBDD409E"
EXPECTED_CAPTURE_SCHEMA_HASH = (
    "95B6444796A21FD44E94113B75BA2097BC381D5F72ED784F9B9A4A99DD46D951"
)
STUDENT_CONTROL_GATE = 0.95

# The bc_v3_a smoke run (its own sidecar log) then the 5 fresh paired-eval runs
# (shared sidecar log), in launch order. Provenance smoke_run=True for the first.
SMOKE_RUN_ID = "run_1784880640_76087"
PAIRED_RUN_IDS = [
    "run_1784882794_20877",  # defeat w20
    "run_1784883926_57395",  # defeat w10
    "run_1784884408_98596",  # defeat w11
    "run_1784884958_1046",   # defeat w20
    "run_1784886119_60496",  # defeat w20
]


# --------------------------------------------------------------------------- #
# Inline Stage-A audit (reused primitives) across the two v3a sidecar logs
# --------------------------------------------------------------------------- #
def verify_sidecar_identity(parsed: dict[str, Any], registry: dict[str, Any], tag: str) -> dict[str, Any]:
    """Verify a torch sidecar startup identity against the bc_v3_a_s1 registry."""
    ...
    log_model = str((startup or {}).get("model_sha256", "")).upper()
    log_norm = str((startup or {}).get("normalization_sha256", "")).upper()
    log_schema = str((startup or {}).get("observation_schema_hash", "")).upper()
    log_name = str((startup or {}).get("registry_run_name", ""))
    checks = {
    ...
    return {
        "log": tag,
        "identity_ok": bool(startup is not None and all(checks.values())),
        "checks": checks,
        "backend": (startup or {}).get("backend"),
...
def audit_v3a_runs(runs_root: Path) -> dict[str, Any]:
    """Run the Stage-A audit inline for the 6 v3a runs against their sidecar logs.

    Returns included run ids (audit order) plus an audit_by_run map carrying the
    student-control + triple-reconciliation stats the manifest embeds."""
    if not REGISTRY_PATH.exists():
        raise FileNotFoundError(f"missing bc_v3_a registry: {REGISTRY_PATH}")
    registry = json.loads(REGISTRY_PATH.read_text(encoding="utf-8"))

    parsed_smoke = load_sidecar(SIDECAR_SMOKE)
    parsed_paired = load_sidecar(SIDECAR_PAIRED)
    id_smoke = verify_sidecar_identity(parsed_smoke, registry, "dagger_r3")
    id_paired = verify_sidecar_identity(parsed_paired, registry, "paired_eval")
    smoke_map = parsed_smoke["act_maps"].get(0, {})
    paired_map = parsed_paired["act_maps"].get(0, {})

    # (run_id, act_map, sidecar_tag, smoke_flag)
    ordered = [(SMOKE_RUN_ID, smoke_map, "dagger_r3", True)] + [
        (rid, paired_map, "paired_eval", False) for rid in PAIRED_RUN_IDS
    ]

    audit_by_run: dict[str, Any] = {}
    included: list[str] = []
    for rid, act_map, tag, is_smoke in ordered:
        entry = audit_run(rid, runs_root, False)
        recon = reconcile_run(rid, runs_root, act_map, False)
        reasons = list(entry["reasons"])
        if not recon["reconciliation_ok"]:
            if recon["student_missing_act"]:
                reasons.append(f"{recon['student_missing_act']} student ticks missing sidecar act")
            if recon["ok"]["violations"]:
                reasons.append(f"{recon['ok']['violations']} ok-tick proposal!=executed (>1e-4)")
            if recon["clamped"]["violations"]:
                reasons.append(f"{recon['clamped']['violations']} clamped-tick renorm violations")
        ...
        audit_by_run[rid] = entry
        if verdict == "include":
            included.append(rid)
        print(
            f"  audit {rid} (smoke={is_smoke}, log={tag}): verdict={verdict} "
            f"ctrl={entry['student_control_fraction']:.4f} wave={entry['wave_reached']} "
            f"{entry['result']} join={recon['join_coverage']:.4f} recon_ok={recon['reconciliation_ok']}",
            flush=True,
        )

    return {
        "included": included,
        "audit_by_run": audit_by_run,
        "sidecar_identity": {"smoke": id_smoke, "paired": id_paired},
        "sidecar_act_counts": {
            "dagger_r3": len(smoke_map),
            "paired_eval": len(paired_map),
        },
    }


# --------------------------------------------------------------------------- #
# Build (mirrors r2 build(); r3 provenance + two-log reconciliation roll-up)
# --------------------------------------------------------------------------- #
def build(
    run_ids: list[str],
    schema: dict[str, Any],
    ...
    for run_id in run_ids:
        ...
        t0 = time.time()
        entry = build_run(run_id, events_path, schema, dataset_dir)  # frozen Stage B
        shard_path = dataset_dir / entry["shard_file"]
        with np.load(shard_path) as shard:
            derived = compute_aux_and_weights(shard, contact_idx, hp_idx)  # frozen Stage C
        ...
        entry["sidecar_log"] = audit.get("sidecar_log")
    ...
    # Triple-reconciliation roll-up (both logs), plus a per-log breakdown.
    def _empty_recon() -> dict[str, Any]:
        return {
            "student_ticks": 0, "student_matched": 0, "student_missing_act": 0,
            "ok_ticks": 0, "ok_violations": 0, "ok_max_abs_delta": 0.0,
            "clamped_ticks": 0, "clamped_violations": 0,
            "clamped_max_dir_diff": 0.0, "clamped_max_mag_err": 0.0,
            "timeout_ticks": 0, "timeout_with_act": 0, "not_connected_ticks": 0,
            "all_reconciliation_ok": True,
        }

    def _accumulate(target: dict[str, Any], rc: dict[str, Any]) -> None:
        target["student_ticks"] += rc["student_ticks"]
        target["student_matched"] += rc["student_matched"]
        target["student_missing_act"] += rc["student_missing_act"]
        target["ok_ticks"] += rc["ok"]["ticks"]
        target["ok_violations"] += rc["ok"]["violations"]
        target["ok_max_abs_delta"] = max(target["ok_max_abs_delta"], rc["ok"]["max_abs_delta"])
        target["clamped_ticks"] += rc["clamped"]["ticks"]
        target["clamped_violations"] += rc["clamped"]["violations"]
        target["clamped_max_dir_diff"] = max(target["clamped_max_dir_diff"], rc["clamped"]["max_dir_diff"])
        target["clamped_max_mag_err"] = max(target["clamped_max_mag_err"], rc["clamped"]["max_mag_err"])
        target["timeout_ticks"] += rc["timeout_ticks"]
        target["timeout_with_act"] += rc["timeout_with_act"]
        target["not_connected_ticks"] += rc["not_connected_ticks"]
        target["all_reconciliation_ok"] &= bool(rc["reconciliation_ok"])

    recon_agg = _empty_recon()
    recon_by_log: dict[str, dict[str, Any]] = {}
    for e in run_entries:
        rc = e.get("triple_reconciliation") or {}
        if not rc:
            continue
        _accumulate(recon_agg, rc)
        log = e.get("sidecar_log") or "unknown"
        recon_by_log.setdefault(log, _empty_recon())
        _accumulate(recon_by_log[log], rc)
    recon_agg["join_coverage"] = (
        round(recon_agg["student_matched"] / recon_agg["student_ticks"], 6)
        if recon_agg["student_ticks"] else 0.0
    )
    for log, rc in recon_by_log.items():
        rc["join_coverage"] = (
            round(rc["student_matched"] / rc["student_ticks"], 6) if rc["student_ticks"] else 0.0
        )
    ...
    gates = [
        {"name": "all_shards_written",
         ...
         "value": shards_written, "threshold": len(run_ids),
        ...
        {"name": "valid_and_temporal_positive",
         ...
         "value": agg["valid_and_temporal_valid"], "threshold": 1,
        ...
        {"name": "aux_weights_aligned",
         ...
         "value": shards_written, "threshold": shards_written,
        ...
        {"name": "triple_reconciliation_ok",
         ...
         "value": recon_agg["join_coverage"], "threshold": 1.0,
         "detail": (f"join_cov={recon_agg['join_coverage']}, ok_viol={recon_agg['ok_violations']}, "
    ...
    common_provenance = {
        ...
        "source_stage": "M4 paired evaluation (design sect. 9) - v3a arm",
        "sidecar_log_smoke": str(SIDECAR_SMOKE),
        "sidecar_log_paired": str(SIDECAR_PAIRED),
        "note": (
            "Eval runs, not a collection campaign: Stage-A audit computed inline "
            "by the builder (no dedicated audit JSON). Per design sect. 9 these "
            "6 runs enter the round-3 corrective pool as valid DAgger data under "
            "bc_v3_a regardless of the retain-bc_v2_f promotion verdict."
        ),
    }

    manifest = {
        "dataset_id": "combat_dagger_r3",
        ...
        "reused_audit_helpers": "scripts/wp2_dagger_r2_run_audit.py::{load_sidecar,reconcile_run}",
        "repo_head_commit": head,
        "runs_root": str(runs_root),
        "run_count": len(run_ids),
        "student_policy": STUDENT_POLICY,
        "student_model_sha256": EXPECTED_MODEL_SHA256,
        "source_campaign": common_provenance,
        "corrective_config": {
        ...
        "triple_reconciliation_by_log": recon_by_log,
        "sidecar_identity": SIDECAR_IDENTITY_HOLDER["value"],
        "sidecar_act_counts": SIDECAR_IDENTITY_HOLDER["act_counts"],
        "gates": gates,
...
# Small module-level holder so build() can embed the inline audit's sidecar
# identity block without threading it through every signature.
SIDECAR_IDENTITY_HOLDER: dict[str, Any] = {"value": None, "act_counts": None}


# --------------------------------------------------------------------------- #
# Report rendering (r2 layout + r3 two-log reconciliation section)
# --------------------------------------------------------------------------- #
def render_md(report: dict[str, Any], manifest_sha: str) -> str:
    lines: list[str] = []
    lines.append("# combat_dagger_r3 corrective dataset report")
    lines.append("")
    ...
    lines.append("")
    idn = report.get("sidecar_identity") or {}
    lines.append("## Sidecar identity (both v3a logs)")
    lines.append("")
    for key in ("smoke", "paired"):
        side = idn.get(key) or {}
        lines.append(
            f"- {key} (`{side.get('log')}`): identity_ok **{side.get('identity_ok')}** | "
            f"backend `{side.get('backend')}` | run `{side.get('registry_run_name')}` | "
            f"handshakes {side.get('handshake_run_ids')} | serving {side.get('serving')}"
        )
    lines.append("")
    lines.append("## Gates")
    lines.append("")
    ...
    lines.append("")
    lines.append("## Triple reconciliation (aggregate over both logs)")
    lines.append("")
    ...
    lines.append("")
    lines.append("Per-log breakdown:")
    lines.append("")
    lines.append("| sidecar log | student ticks | join_cov | ok_viol | clamp_viol | missing | all_ok |")
    lines.append("| --- | --- | --- | --- | --- | --- | --- |")
    for log, r in (report.get("triple_reconciliation_by_log") or {}).items():
        lines.append(
            f"| {log} | {r['student_ticks']:,} | {r['join_coverage']} | {r['ok_violations']} | "
            f"{r['clamped_violations']} | {r['student_missing_act']} | {r['all_reconciliation_ok']} |"
        )
    lines.append("")
    ...
    lines.append("")
    lines.append(
        "| run_id | smoke | log | rows | valid&temporal | episodes | dmg_rate | ctrl_frac | "
    ...
    for e in report["runs"]:
        ...
        lines.append(
            f"| {e['run_id']} | {e['smoke_run']} | {e['sidecar_log']} | {e['counts']['total']:,} | "
...
# --------------------------------------------------------------------------- #
def main() -> int:
    parser = argparse.ArgumentParser(description="Build combat_dagger_r3 corrective dataset (v3a arm)")
    ...
    try:
        ...
        print("Stage-A inline audit of the 6 bc_v3_a runs (two sidecar logs) ...", flush=True)
        audit = audit_v3a_runs(args.runs_root)
        SIDECAR_IDENTITY_HOLDER["value"] = audit["sidecar_identity"]
        SIDECAR_IDENTITY_HOLDER["act_counts"] = audit["sidecar_act_counts"]
        ident = audit["sidecar_identity"]
        if not (ident["smoke"]["identity_ok"] and ident["paired"]["identity_ok"]):
            print("error: bc_v3_a sidecar identity mismatch", file=sys.stderr, flush=True)
            print(f"  smoke: {ident['smoke']['checks']}", file=sys.stderr, flush=True)
            print(f"  paired: {ident['paired']['checks']}", file=sys.stderr, flush=True)
            return 1
        included = audit["included"]
        if len(included) != 6:
            print(f"error: expected 6 included runs, got {len(included)}: {included}",
        ...
        print(
            f"Building combat_dagger_r3 over {len(included)} runs -> {dataset_dir} "
            f"(smoke={args.smoke})",
            flush=True,
        )
        result = build(included, schema, args.runs_root, dataset_dir, audit["audit_by_run"])
